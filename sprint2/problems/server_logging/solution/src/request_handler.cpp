...
namespace http_handler {
    ...
    http::response<http::empty_body> RequestHandler::CopyResponseWithoutBody(const ResponseVariant& response) const {
        // Создаем новый ответ
        http::response<http::empty_body> new_response;

        // Обработка variant с помощью std::visit
        std::visit([&](auto&& arg) {
            using T = std::decay_t<decltype(arg)>;
            // Копируем результат, версию и заголовки
